...
/**
 * **발판(소리·앵커 낱말)은 이 스키마에 없다.** 전에는 `hook: { word, mark }` 가 **필수** 칸이었고,
 * 필수 칸은 모델이 "없다" 고 답할 길을 막는다 — 문구는 부탁이고 스키마는 강제다. 그래서 앵커 낱말이
 * 없는 한자에도 모델이 낱말 하나를 지어냈다. 발판은 사전에서 온다 (`app/inputs/[id]/actions.ts`).
 */
...
/**
 * **부품이 없는 한자에 무엇을 적을지가 빠져 있었다.** 규칙은 "부품의 뜻을 이어 한 구절로" 하나였는데,
 * 사용자 줄은 부품이 없으면 `부품: 없음(한 덩어리)` 로 나간다 — **이을 것이 없는 자리에 이으라고만
 * 말한 것**이다. 2,136자 중 **457자**(21%)가 그 자리고, 그 안에 日·大·人·会·生 처럼 어느 기사에나
 * 나오는 글자가 들어 있다. 무엇이 나올지는 아무도 안 봤다 — 표본 0.
 *
 * 손으로 적은 다섯 장(世 車 日 力 必)이 이미 답을 하고 있었다. 그래서 새 규칙을 지어내지 않고
 * **그들이 하던 것을 그대로 적었다** — 日 "해의 모양", 車 "바퀴 달린 수레를 위에서 본 모양",
 * 力 "팔에 힘을 준 모양", 必 "한 획이 가운데를 가로지른 모양". 부품이 있는 1,679자의 문안은
 * 이 갈래를 안 지나므로 그대로다.
 *
 * **다섯이 아니라 넷이다. 世 는 예시로 쓰지 않는다.** 그 한 장만 "열(十)이 셋, 서른 해" 로
 * **쪼개서** 푸는데, `kanji.json` 에서 世 의 `parts` 는 다른 넷과 똑같이 `[]` 다. 어원으로는 맞는
 * 말이지만 **우리가 모델에게 주는 것이 아니다** — 지어낸 부품이다. 처음 적을 때 그것을 예시 셋
 * 중 하나로 넣었는데, **"부품을 지어내지 마라"고 말하는 바로 그 줄이 지어낸 부품을 보여 주고
 * 있었다.** 모델은 금지 문장보다 예시를 세게 읽으므로, 大 에서 "一 과 人 이 겹친 것" 같은 것이
 * 나오고 그건 그럴듯해서 **보고도 못 잡는다.** 예시를 바꾸는 대신 넷으로 줄였다 — 넷이 이미
 * 같은 말을 한다. (世 의 문안 자체는 손으로 적힌 것이라 이 갈래를 안 지나므로 그대로 둔다.)
 *
 * **뜻을 먼저 말하지 말라고 같이 못 박는다.** 모양을 적으라고만 하면 "해와 날을 뜻하는 글자" 로
 * 빠지기 쉬운데, 그러면 Scene4 가 이미 답을 본 뒤의 추측이 된다 (CLAUDE.md 원칙 1).
 */
...
- 부품이 "없음(한 덩어리)" 이면 **부품을 지어내지 말고 글자의 모양**을 한 구절로 적는다. 예: 日 → "해의 모양", 車 → "바퀴 달린 수레를 위에서 본 모양", 力 → "팔에 힘을 준 모양". 없는 부품을 끌어오지 않는다 — "十이 셋" 처럼 쪼개지 않는다. 뜻을 먼저 말하지 않는다 — 그 모양에서 question 이 출발한다.
- question: parts_meaning 에서 출발해 사용자가 먼저 뜻을 추측하게 하는 질문 한 줄. 정답을 말하지 않는다. 형식 "…이면\\n무슨 뜻이 될까?" (줄바꿈 한 번).
- answer: 이 한자의 뜻을 한국어 한 줄로 (5~12자). 문법·규칙 설명 금지.
- landing: 이 한자가 든, 한국어 화자가 이미 아는 한자어 2개. 일본어 표기(word), よみがな(reading, 히라가나), 한국어(ko). 앵커 낱말이 주어졌으면 그것을 첫 번째로. 한국어와 일본어가 같은 두 글자 조합만.
...
export async function getCardContent(
  ...
  try {
    ...
    const user = [
      ...
      `음독: ${(m.on ?? []).join(", ") || "-"}`,
      `한국 한자음: ${m.ko_sound ?? "-"}`,
      // **없으면 "없음" 이라고 말한다.** 전에는 이 줄이 "흔한 한자어를 하나 골라라" 로 끝나서, 앵커
      // 낱말이 없는 한자마다 모델이 하나를 지어냈다. 그 낱말이 F03 의 "부를 낱말이 아직 없어" 를
      // 다음 화면에서 뒤집었다 (design/SCREENS.md "부를 낱말이 없는 한자의 카드").
      `앵커 낱말 (이미 아는 한국어 한자어): ${m.ko_word ?? "없음"}`,
