  ...
  ja: {
    ...
    /*
      ...
      **이 자료로는 「아는 것 → 다음 것」 한 줄이 여섯 번째 카드에서야 선다. 버그가 아니다.**
      ...
      까닭은 본문에 있다. `協` 의 부품은 `十`·`力` 인데 **그 둘을 나누는 글자가 이 본문에 하나도
      없다** — 씨앗의 `parts` 를 스무 자 전부 맞대어 봤다. 그래서 `nextCandidates` 의 `shared` 가
      계속 비고 `via` 가 `null` 이라, 순서는 앵커 낱말 반 점(0.5)과 자료에 나온 차례가 정한다.
      첫 이유 한 줄은 **`亻` 를 나누는 자리**에서 선다(`代` → `作`).
