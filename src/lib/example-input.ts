...
export const EXAMPLE_INPUT: Record<Lang3, { name: string; title: string; body: string }> = {
  ja: {
    ...
    /*
      ...
      **이 자료로는 「아는 것 → 다음 것」 한 줄이 여섯 번째 카드에서야 선다. 버그가 아니다.**
      앞 다섯은 이유 자리에 **자료를 대는 대체 문구**가 선다(`docs/FLOW.md` 4장 「다음 카드 이유 한 줄은
      "아는 것 → 다음 것" 한 문장이다」의 끝: "다음은 支. 아침 기사에서."). 화면이 비는 게 아니라 **주어로 댈 판정된 한자가 아직 없는 것**이다.

      **까닭은 본문이 아니라 범위에 있다.** 한때 여기 "까닭은 본문에 있다" 라고 적혀 있었는데
      **원인 귀속이 틀렸다.** 본문 안에서 참인 것은 맞다 — `協` 의 부품 `十`·`力` 을 나누는 글자가
      이 스무 자 안에 하나도 없어서 `nextCandidates` 의 `shared` 가 계속 비고 `via` 가 `null` 이다.
      **틀린 것은 그걸 이 자료의 성질로 읽은 것이다.** 씨앗 2,136자 중 `力` 을 부품으로 가진 한자는
      **27자**인데(動 務 加 男 助 労 働 効 … `docs/SPEC.md` 「복습 없는 복습」의 加·助·動 이 그 안에 있다) 그중
      이 본문에 있는 것은 `協` 하나다. 나머지 26자는 그래프에서 한 칸 거리인데 **`nextCandidates`
      가 후보를 `prog.nodes`(자료 한 편에 나온 한자)에서만 뽑아서 닿을 길이 없다.** 그러니
      **자료를 바꿔도 이 모양은 다음 자료에서 그대로 다시 난다** — 고칠 자리는 범위다.

      **그 판정은 이미 났다.** `CLAUDE.md` 「앵커 그래프 데이터 모델」의 「다음 카드 = 아는 노드에서
      가장 가까운 모르는 노드」: 「고르는 함수는 `inputs` 가 아니라 `user_node_state`
      (아는 노드)에서 출발한다 — 인풋은 후보 집합에 가중치를 더할 뿐이고, 인풋이 0개여도 함수가
      돈다」(`85c3a94`). **지금 코드는 아직 그 앞이다** — `nextCandidates` 가 `prog.nodes` 에서만
      뽑는 한 위 모양은 남는다. 그러니 **이 주석이 설명하는 것은 고쳐질 상태**이고, 범위가 바뀌는
      날 이 문단은 통째로 지워진다. 그때까지 예시 자료를 손대는 것으로 고치려 들지 마라.

      그래서 순서는 앵커 낱말 반 점(0.5)과 자료에 나온 차례가 정하고, 첫 이유 한 줄은
      **`亻` 를 나누는 자리**에서 선다(`代` → `作`).

      **조건이 하나 붙는다: 이 수는 F03 에서 아무것도 「알아」로 안 고르고 카드만 따라간 길의
      값이다.** 하나라도 고르면 그게 앵커가 되어 더 일찍 설 수 있다.

      잰 방법: `nextCandidates` 를 그대로 부르면서 매번 최고점을 켜 봤다(씨앗 JSON 으로 노드를
      세워 부른 것이라 DB 를 안 탄다). **안 적으면 「이유 한 줄이 안 나온다」로 올라오고, 받은
      사람이 처음부터 다시 잰다.**
