...
export type InputRowData = {
   ...
   *
   * **「아직 안 뽑았다」 는 한자가 하나도 없는 자료까지 참으로 만든다** (`prog.nodes.length === 0`).
   * 한자 없는 일본어 글이나 사전에 없는 글자만 든 글이 그렇다. 그런 자료는 눌러도 뽑을 게 없는데
   * 홈에서는 **할 일이 있는 자료와 같은 얼굴로** 서고, 넷뿐인 자리를 먼저 차지한다.
   * 시험 데이터를 만들다 본 것이지만 **제품에서도 일어난다** — 적어 두는 이유가 그거다.
   * 고치려면 "안 뽑았다" 와 "뽑을 게 없다" 를 갈라야 하는데, 그건 화면이 그 둘을 다르게 부를 말이
   * 있어야 하는 일이라 기획 몫이다.
   */
  actionable: boolean;
};
