...
export default async function TodayPage({ searchParams }: { searchParams: Promise<{ fixed?: string }> }) {
  ...
  /*
    ...
    **「유일한 입구라 밀리면 통째로 막힌다」고 적혀 있었는데 틀렸다** (2026-09-22 PM 축이 재고
    잡았다). `/inputs`(F19)가 **같은 `toInputRow`** 로 행을 만들고 그 함수가 다 본 일본어 자료에
    `/inputs/{id}/read` 를 붙인다. 그러니 막히는 게 아니라 **한 탭 깊어진다** — 홈 → 지난 자료 →
    그 자료 → F12.

    **잰 값은 이렇다** (11:08 UTC, `splitForHome` 을 직접 돌렸다). 자리가 남으면 다 본 것 하나를
    올리므로 **둘째 날은 어제 것이 홈에 바로 뜬다.** 밀리는 날은 **actionable 자료가 넷이 되는
    날**이고, 그때 홈은 길을 막지 않고 **「어제 것이 여기 있다」고 말하지 않을** 뿐이다.

    **그래도 넷으로 자르는 판단은 그대로다** — 까닭은 낭떠러지가 아니라 **홈이 목록이 되면 안
    된다**는 것이고 그건 `docs/FLOW.md` 4장에 있다. **없는 낭떠러지를 적어 두면 다음에 `HOME_MAX`
    를 건드리는 손이 안 위태로운 것을 보고 물러서거나, 위태롭다고 읽고 엉뚱한 것을 고치러 간다.**

    **남는 값 하나** — 밀리는 쪽이 하필 **통과 기준이 걸린 축**(2주 후 자료 재만남 70%)이고,
    2주째가 정확히 자료가 여럿 쌓이는 때다. 영어 축의 「못 한 말」 행은 자료 수와 무관하게 늘 선다.
  */
