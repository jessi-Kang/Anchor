 ...
 * **이 화면이 일본어 축 재만남(F12)의 둘째 입구다.** 한동안 여기 「다 본 자료 행 하나가 유일한
 * 입구」라고 적혀 있었는데 **이 파일이 그 문장의 반례였다** — `toInputRow` 가 다 본 일본어 자료에
 * `/inputs/{id}/read` 를 붙이고, 이 화면이 그 함수를 홈과 똑같이 쓴다. **제 존재를 부정하는 주석을
 * 제 머리에 달고 있었다** (2026-09-22 PM 축이 재고 잡았다).
 *
 * **그러니 홈에서 밀려도 막히지 않고 한 탭 깊어진다.** 밀리는 조건은 **actionable 자료가 넷**일
 * 때고(`splitForHome` — 자리가 남을 때만 다 본 것 하나를 올린다), 그게 이 화면이 뜨는 조건과
 * 정확히 같다. 통과 기준("2주 후 **자료** 재만남 인식률 70%", `docs/SPEC.md` 9장) 둘 중 하나가
 * 이 길에 걸려 있고, **2주째가 자료가 넷을 넘는 때**라 이 화면은 그때 실제로 쓰인다.
 *
