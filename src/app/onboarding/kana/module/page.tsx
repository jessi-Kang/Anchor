 ...
 */
export default async function KanaModulePage({ searchParams }: { searchParams: Promise<{ next?: string; why?: string; fixed?: string }> }) {
  const { next: rawNext, why, fixed } = await searchParams;
  if (!isDesignPreview()) {
  ...
  const mic = why === "mic";
  return (
    <Screen where="가나" up="/today" aside={isDesignPreview() ? "오전 8:43" : nowKST()} fixed={fixed === "1"}>
      <Grow />
