...
export async function GET(req: Request) {
  ...
  // 204 를 주는 조건은 화면이 "들려줄 소리가 없다" 고 말하는 조건과 **같아야 한다** → lib/tts-voice.ts.
  // 목소리 고르기도 거기 한 곳에서 한다: 언어별 목소리가 먼저, 없으면 Jessi 목소리 클론.
  const voice = mine ? process.env.ELEVENLABS_VOICE_ID_JESSI : targetVoiceId(lang);
