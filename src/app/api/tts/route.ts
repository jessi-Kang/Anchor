...
/**
 * GET /api/tts?text=協力&voice=mine|default — ElevenLabs TTS (mp3).
 * 키가 없으면 204: 클라이언트가 브라우저 음성(SpeechSynthesis ja-JP)으로 대신한다.
...
export async function GET(req: Request) {
  ...
  const voice = (mine ? process.env.ELEVENLABS_VOICE_ID_JESSI : process.env.ELEVENLABS_VOICE_ID_JA) ?? process.env.ELEVENLABS_VOICE_ID_JESSI;
  ...
  const res = await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${voice}?output_format=mp3_44100_64`, {
    ...
    body: JSON.stringify({ text, model_id: "eleven_multilingual_v2", language_code: "ja" }),
  });
  if (!res.ok || !res.body) {
