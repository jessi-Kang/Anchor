...
- [ ] **인풋 0개에서 전부 돈다**: 카드 · 그래프 · 재만남(내장 예문 안에서) · 대화 루프(상황 템플릿에서 제안). 다음 카드를 고르는 함수가 `inputs` 가 아니라 `user_node_state` 에서 출발하고, 인풋은 후보에 가중치만 더한다
  ...
  - **보류된 칸은 메우고 싶어진다. 그 압력이 이 문서의 위험이다.** 2026-09-22 하루에 두 번 났다 — 나는 「내장 코퍼스에 무엇을 담을지 짜서 올리겠다」고 했고(접었다), 측정 쪽은 **아직 설계 안 된 장치의 동작을 전제하고** 그 위에서 편향 크기를 단정했다(고쳤다). **둘 다 없는 장치의 내용을 먼저 정한 것이다.** 빈칸을 그대로 두는 것이 일을 안 하는 것처럼 보이지만, **메운 것은 잘리는 날 통째로 버려지고 그 사이에 남이 그 위에 쌓는다.** 빈칸에는 **왜 비어 있는지와 무엇이 정해져야 채워지는지**만 적는다
  - **2026-09-22, 「주제별 코스와 뭐가 다른가」로 걸렸던 의문은 닫혔다**(`README.md` 근간). **판정: 상황은 후보 집합을 만들고 순서는 여전히 아는 노드가 정한다** — 정렬이면 코스고 집합이면 다르다. **그리고 그 판정이 이 칸을 더 무겁게 만든다: 「안 다르다」의 원인은 상황이 아니라 범위였다.** 상황을 빼도 안 달라진다 — 후보를 자료 한 편에서만 뽑으면 고른 자리에 닿을 것이 없다. **이 칸이 바로 그 자리다**
