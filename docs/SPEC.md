...
2. 복습 없는 복습: 力을 배운 뒤 加, 助, 動이 나오면 力이 저절로 다시 보인다. 카드를 다시 보여줄 필요가 없다. **그 "나오면"은 자료를 기다리는 말이 아니다** — 그래프가 내장 코퍼스에서 골라도 나온다. 후보를 자료 안에서만 고르면 이 줄이 구조적으로 막힌다(실제로 `加`·`助` 가 예시 기사에 없어 막혀 있었다).
...
- [ ] **인풋 0개에서 전부 돈다**: 카드 · 그래프 · 재만남(내장 예문 안에서) · 대화 루프(상황 템플릿에서 제안). 다음 카드를 고르는 함수가 `inputs` 가 아니라 `user_node_state` 에서 출발하고, 인풋은 후보에 가중치만 더한다
  - 남은 것: **전부.** 내장 코퍼스가 아직 없고(씨앗은 사전이지 상황별 큐가 아니다), 지금 큐는 자료에서만 나온다. **빈 상태에 "자료를 넣어야 시작할 수 있어요"가 뜨면 버그다**
  ...
  - **이 칸을 닫는 조건.** 폭 **390px** 에서 첫 방문을 처음부터 끝까지 한 번 밟고, **지나는 화면마다** 둘을 본다. **(가) 주 버튼이 스크롤 없이 보이는가. (나) 나가는 길이 눌러 보이는가** — 그 화면에서 홈으로 가는 것을 **모르는 사람이 찾을 수 있는가**. **한 화면이라도 아니면 이 칸은 안 켜진다.** 「실제 기기에서 봤다」로 적지 않는다 — 그러면 다음 사람이 폰으로 한 번 열어 보고 켠다. 무엇을 보는지가 조건이다.
...
- [x] 모바일 화면 우선 (통근·점심에 쓰므로) — QA 는 390×844 에서만 결함을 센다
