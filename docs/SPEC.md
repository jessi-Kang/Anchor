...
한자 씨앗 40장은 영어 씨앗과 같은 구조다. 한자 하나를 보고 **소리**가 떠오르는지 판정한다(뜻은 묻지 않는다). 뒤집으면 음독, 예시 단어(よみがな), 한국어 한자어 앵커("협력의 협")만 보인다. 카드 순서는 받침 → 음독 패턴(받침 없음 → ㄴ·ㅁ→ん → ㄱ→く → ㅂ→う → ㄹ→つ → ㅇ→う/い)으로 묶어서, 규칙 표를 보여주지 않아도 순서만으로 패턴이 스스로 드러나게 한다. 판정 결과는 `knows_sound`(한자어 소리를 떠올렸는가)로 들어가고, `knows_meaning`은 발견 카드가 채운다. 선정 기준은 `db/seed/README.md`.
...
| 3′. 한자 씨앗 3분 (일본어를 켰을 때) | 한자 한 장씩. 소리를 먼저 떠올리고 → 뒤집어 음독·예시 단어 확인 → "떠올랐다 / 안 떠올랐다" | 뜻이 아니라 한국어 한자어 소리가 떠오르는지만 본다. 답을 본 뒤 판정 |
