...
세 언어를 한 번에 시작하지 않아도 된다. 지금 쓸 언어 하나만 켜고, 나중에 설정에서 다른 언어를 켠다. 온보딩은 켠 언어에 필요한 단계만 돈다. 나중에 언어를 켜면 그 언어의 단계만 이어서 한다. 그래프는 언어 공통 테이블이므로 나중에 켠 언어도 이미 아는 노드(예: 영어 어근 → 스페인어 코그네이트) 위에 바로 올라간다.
...
**온보딩 (켠 언어에 따라 2~5장, 5분 안에 끝)**

| 화면 | 하는 일 | 왜 자기 보고가 아닌가 |
| --- | --- | --- |
| 1. 로그인 | Google 로그인. 데이터는 계정별로 분리, 기본 비공개 | 첫 사용자는 Jessi 하나지만, 구조는 처음부터 다수 계정 |
| 2. 목적 | 지금 쓸 언어의 "어디서 쓰나"만 탭 (업무, 수업, 콘텐츠, 여행). 상황을 하나라도 고른 언어가 켜진다. 대화는 기본이라 묻지 않음 | 상황을 고르는 것이지 수준을 고르는 게 아님 |
| 3. 가나 읽기 (일본어를 켰을 때) | 가나 6개를 보고 소리 내서 읽기. 못 읽으면 가나 모듈로 | "읽을 수 있다"고 말하는 대신 실제로 읽는다 |
| 3′. 한자 씨앗 3분 (일본어를 켰을 때) | 한자 한 장씩. 일본어 읽기를 먼저 떠올리고 → 뒤집어 읽기·예시 단어 확인 → "떠올랐다 / 안 떠올랐다" | "아는가?"가 아니라 "읽기가 떠올랐는가?"를 본다. 답을 본 뒤 판정 |
| 4. 영어 씨앗 3분 (영어·스페인어를 켰을 때) | 빈도 높은 어근·덩어리 한 장씩. 뜻을 먼저 떠올리고 → 뒤집어 확인 → "맞았다 / 틀렸다" | "아는가?"가 아니라 "떠올렸는가?"를 묻는다. 답을 본 뒤 판정하므로 과대평가가 어렵다 |
