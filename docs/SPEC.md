...
| 일본어 | 가나(히라가나·가타카나)를 읽을 수 있음 | 가나 모듈(v2)로 분기. 한자 카드는 가나를 못 읽으면 성립하지 않음 | "읽을 수 있나요?"가 아니라 가나 6개를 보여주고 소리 내서 읽게 함. 마이크 인식으로 판정 |
...
세 언어를 한 번에 시작하지 않아도 된다. 온보딩의 첫 화면에서 시작할 언어를 고른다(여러 개 가능, 최소 1). 고른 언어는 한 언어씩 준비한다: 그 언어의 상황 → 그 언어의 단계 → 다음 언어. 나중에 홈이나 설정에서 다른 언어를 켜면 그 언어의 단계만 이어서 한다. 그래프는 언어 공통 테이블이므로 나중에 켠 언어도 이미 아는 노드(예: 영어 어근 → 스페인어 코그네이트) 위에 바로 올라간다.

언어별 단계(가나 읽기·한자 씨앗·영어 씨앗)는 "나중에 할게"로 건너뛸 수 있다. 홈에 들어가는 조건은 "언어를 하나라도 켰다"뿐이다. 건너뛴 단계는 홈과 설정의 언어 행에 "가나 읽기 남음"처럼 남고, 탭하면 이어서 한다. "여기까지"로 끝낸 단계는 마친 것으로 보고 다시 묻지 않는다. 단계 상태는 언어가 아니라 사용자 단위다: 영어 씨앗은 영어·스페인어가 같은 40장을 공유하므로, 스페인어에서 끝냈다면 나중에 영어를 켜도 상황만 고르고 바로 홈이다.

| 켠 언어 | 필요한 단계 | 이유 |
| --- | --- | --- |
| 영어 | 영어 씨앗 40장 (어근·덩어리) | 영어 그래프의 첫 "아는 노드" |
| 일본어 | 가나 6개 읽기 → 한자 씨앗 40장 | 가나는 문지방(한자 카드는 가나를 못 읽으면 성립하지 않음). 한자 씨앗은 그래프의 첫 "아는 한자". 이게 없으면 첫 한자 카드가 "아는 것 + 1"로 시작할 발판이 없다 |
| 스페인어 | 영어 씨앗 40장 (영어와 같은 것, 뒤집으면 스페인어 대응을 함께 보임) | 스페인어 앵커는 영어 라틴 어근. 영어를 이미 켰으면 추가 단계 없음 |

세 언어 공통 바닥은 영어 기초(알파벳, 기초 문장)다. 스페인어는 영어 어근 위에 서므로, 영어를 전혀 읽지 못하는 사용자는 현재 범위 밖이다. 스페인어만 켠 사용자에게는 이 전제를 화면에서 한 줄로 알린다.

한자 씨앗 40장은 영어 씨앗과 같은 구조다. 한자 하나를 보고 **일본어 읽기**가 떠오르는지 판정한다. 뒤집으면 읽기(きょう)와 예시 단어(協力 きょうりょく)만 보인다. 한국어는 나오지 않는다. 한국어 한자어 소리는 한국어 화자면 이미 아는 것(아래 "안다는 세 층")이라 묻지 않고, 발견 카드의 후킹에서만 쓴다. 카드 순서는 받침 → 음독 패턴(받침 없음 → ㄴ·ㅁ→ん → ㄱ→く → ㅂ→う → ㄹ→つ → ㅇ→う/い)으로 묶어서, 규칙 표를 보여주지 않아도 순서만으로 패턴이 스스로 드러나게 한다. 판정 결과는 `knows_meaning`(그 글자를 일본어 단어로 아는가)으로 들어간다. `knows_sound`(한국어 한자어 소리)는 한국어 화자 기본값 true 다. 선정 기준은 `db/seed/README.md`.

**온보딩 (켠 언어에 따라 2~6장, 5분 안에 끝. 단계는 건너뛰고 홈에서 이어서 할 수 있다)**
...
| 2′. 상황 (언어마다) | 그 언어의 "어디서 쓰나"만 탭 (업무, 수업, 콘텐츠, 여행). 대화는 기본이라 묻지 않음. 상단에 "일본어 1 / 3" | 상황을 고르는 것이지 수준을 고르는 게 아님 |
| 3. 가나 읽기 (일본어) | 가나 6개를 보고 소리 내서 읽기. 못 읽으면 가나 모듈로. "나중에 할게"로 건너뛸 수 있음 | "읽을 수 있다"고 말하는 대신 실제로 읽는다 |
| 3′. 한자 씨앗 3분 (일본어) | 한자 한 장씩. 일본어 읽기를 먼저 떠올리고 → 뒤집어 읽기·예시 단어 확인 → "떠올랐다 / 안 떠올랐다" | "아는가?"가 아니라 "읽기가 떠올랐는가?"를 본다. 답을 본 뒤 판정 |
| 4. 영어 씨앗 3분 (영어·스페인어) | 빈도 높은 어근·덩어리 한 장씩. 뜻을 먼저 떠올리고 → 뒤집어 확인 → "맞았다 / 틀렸다" | "아는가?"가 아니라 "떠올렸는가?"를 묻는다. 답을 본 뒤 판정하므로 과대평가가 어렵다 |
...
| 음성 | 녹음은 계정 전용 저장소에 암호화 저장. 곡선 비교에 필요한 피치 데이터만 남기고 원본은 기본 30일 후 삭제(설정 가능) |
| 백업 | DB는 Point-in-time recovery(최소 7일) + 매일 스냅샷을 별도 스토리지에 보관. 배포 전 자동 백업. 복구 절차를 문서화하고 분기마다 실제로 복구 테스트 |
...
| --- | --- | --- | --- | --- |
| MVP | 3주 | 두 축 동시. (1) 영어 대화 루프 초기판: 오늘 못 한 말 한 줄 → 덩어리 → 듣고 따라 말하고 곡선 비교. (2) 일본어 한자 발견 카드 + 앵커 그래프 | "추측 → 확인"이 암기보다 남는다. 곡선 비교가 말하기를 바꾼다 | 재만남 인식률 70%, 같은 덩어리 5회차 일치도 상승 |
...
1. 대화가 기본 목표라면 말하기 루프가 첫 버전에 없으면 안 된다. 영어부터 하는 건 매일 말해야 하는 상황이 있어서 인풋이 끊기지 않기 때문이다.
...
- [ ] 일본어: 한국어 한자어 또는 일본어 문장 입력 → 모르는 한자 추출
...
| 백업 | Neon PITR(7일 이상) + 매일 스냅샷 내보내기를 별도 스토리지에 | 학습 기록 유실 = 즉시 이탈. 분기마다 복구 테스트 |
| 음성 저장 | 계정 전용 암호화 스토리지, 원본은 30일 후 삭제 | 목소리는 가장 민감한 데이터 |
