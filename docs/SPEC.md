...
2. 복습 없는 복습: 力을 배운 뒤 加, 助, 動이 나오면 力이 저절로 다시 보인다. 카드를 다시 보여줄 필요가 없다. **그 "나오면"은 자료를 기다리는 말이 아니다.** 후보를 자료 안에서만 고르면 이 줄이 구조적으로 막힌다 — 씨앗 2,136자 중 `力` 을 부품으로 가진 한자는 **27자**인데 예시 자료에 나오는 한자 24자 안에 든 것은 **`協` 하나**고 `加`·`助`·`動` 은 전부 밖이다. **자료를 더 좋은 것으로 바꿔서 푸는 문제가 아니다**(9장의 보류된 범위 물음).
...
- [ ] **인풋 0개에서 전부 돈다**: 카드 · 그래프 · 재만남(내장 예문 안에서) · 대화 루프(상황 템플릿에서 제안). 다음 카드를 고르는 함수가 `inputs` 가 아니라 `user_node_state` 에서 출발하고, 인풋은 후보에 가중치만 더한다
  ...
  - **결함의 이름은 「내장 코퍼스가 없다」가 아니라 「큐를 고르는 범위가 자료 한 편이다」다.** 처음에 나는 이 칸을 「자료가 없는 날 무엇을 줄까」로 적었는데, 그건 장치를 먼저 정하고 물음을 거기 맞춘 것이다. 재 보면 범위 쪽이 맞다: 씨앗 2,136자 중 **`力` 을 부품으로 가진 한자가 27자**인데 예시 자료에 나오는 한자 24자 안에 든 것은 **`協` 하나**고, 6.2 의 `加`·`助`·`動` 은 **전부 자료 밖**이다. 자료를 더 좋은 것으로 바꿔도 이 줄은 안 풀린다 — 한 편이 그래프를 덮을 수 없기 때문이다
