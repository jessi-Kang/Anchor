...
| ~~**`条`·`件` 에 앵커 낱말을 줄 것인가**~~ | **판정 났다 — 안 넣는다** (PM 축, 2026-09-21 08:44). 기획이 앵커 33종을 판정하면서 `条`·`件` → **조건**을 같이 냈는데, 표에 넣지 않고 열린 자리로 올린 뒤 갈렸다 | 넣자는 쪽(`5e0e8f6`)의 근거 셋 중 하나가 **지금은 참이 아니다**: *"표에 낱말이 없으면 모델이 흔한 한자어를 하나 골라 발판으로 쓴다"* — `card-content.ts:99` 가 이미 `${m.ko_word ?? "없음"}` 이고 그 줄은 과거형으로 적혀 있다. 남는 어긋남은 설계 구멍이 아니라 **값 하나**였는데 그것도 없다: 씨앗 `条` 카드에 `hook` 칸이 아예 없고(`3d19317` 에서 28장 모두 걷힘), **밟아서 봤더니** F04 가 `조는 알아. 条만 모르지.`, Scene1 이 `이미 아는 소리 / 조` 다 — 카드가 `조건` 으로 말을 걸지 않는다. **그림 다섯(F03·F03a·F03c·Scene1a·Scene5a)은 그대로 선다.** 되열리는 조건: 예시 본문 셋째 문장을 바꾸거나, 둘째 묶음을 채울 다른 글자를 고를 때 |
