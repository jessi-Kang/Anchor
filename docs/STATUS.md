...
**영어 축은 F17 에서 끊긴다 — 기획이 이미 정해 둔 그 자리다.**

F13("오늘 회의에서 못 한 말 있어?") → F17("영어로 뭐라고 할 것 같아?") → 추측을 내면 키가 없으니 F17a("지금은 영어를 못 만들었어. **쓴 건 저장했어.**")까지는 그대로 간다. **저장은 참이다** — 행에 `situation` 과 `meta.guess` 가 다 들어 있다.

**그런데 그 화면을 떠나면 돌아갈 길이 없다.**

| 어디 | 그 행이 보이나 |
| --- | --- |
| 홈의 "못 한 말" 행 | 안 보인다. 부제가 "한 줄이면 돼"(개수 0)라 **새 줄 쓰기(F13)로만** 간다 |
| F18 지난 덩어리(`/talk/past`) | **"아직 없어."** |

`src/lib/db/chunks.ts` 의 `btrim(text) <> ''` 가 `countChunks`(145행)와 `pastChunks`(265행) 양쪽에서 그 행을 뺀다. 영어 문장이 아직 없으니 `text` 가 빈 칸이라서다. **데이터는 안 없어졌고 화면에서만 사라진다.** 주소를 들고 있으면 열리고, 열면 추측이 그대로 있다.

덤으로 하나 — **F13 에서 보내고 F17 에서 아무것도 안 한 행**도 같은 이유로 안 보인다. 그쪽은 추측조차 없어 아까울 게 없지만, 이런 행이 조용히 쌓인다는 뜻이다.

기획이 `3abc75e` 로 **답을 이미 정해 뒀다**(쓰다 만 줄은 F18 에 서고, 탭하면 F14 가 아니라 F17 로, 알약은 "이어서", `countChunks` 도 같이 세야 한다). **정해졌고 안 들어갔다.** 그림(디자인)이 먼저라고 적혀 있다.
