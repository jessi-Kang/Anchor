...
| 무엇 | 지금 상태 | 다음 한 걸음 |
| --- | --- | --- |
| ~~**M0 거짓말 없애기**~~ | **닫혔다** (`c332153` · `6afb185`). 세 줄 중 앞 두 줄(카드·덩어리 폴백)은 앞선 커밋에서 이미 닫혀 있었고 표만 안 따라왔다. 셋째 줄(홈 h1 시제)은 고쳤다. 같은 패턴을 `src/` 전체에서 훑었고 새로 걸린 것은 없다 | **키를 빼고 한 바퀴 돌아 폴백 화면을 눈으로 보는 것**이 남았다. 지금까지 확인은 전부 코드를 읽은 것이다. 근거와 안 본 것은 `README.md` 의 표 아래에 적었다 |
