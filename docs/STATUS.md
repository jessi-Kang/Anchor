...
| **로컬 Postgres 불안정** | PM 축 보고: 비정상 재시작 10회, 약 15분 간격. **이 세션에서는 재현 안 됐다** — 2026-09-21 05:51~06:07, Postgres 16 을 세워 마이그레이션·씨앗·시험 50개·브라우저 walk 까지 돌리는 동안 **재시작 0회**(로그의 `starting PostgreSQL` 이 내가 켠 한 줄뿐, `not properly shut down` 없음). 표본이 한 판·16분이라 **반증은 아니다** | 원인이 샌드박스 아래라 세션이 못 고친다. 다만 "늘 그렇다" 는 아니다. 재현되면 판정이 흔들리므로 **판정 전에 확인**해야 한다 |
