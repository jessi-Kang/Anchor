...
| **`条`·`件` 에 앵커 낱말을 줄 것인가** | **열려 있다. 2026-09-21 에 한 번 닫힐 뻔했다** | 기획이 앵커 33종을 판정하면서 `条`·`件` → **조건**을 같이 냈는데, 표에 **넣지 않고 두었다**(`2c6ccf6`). 나머지 서른여덟 자리는 규칙 1·2·3 을 어긴 줄을 고친 것이라 판단이 없지만, 이 둘은 표에 **없던** 글자라 어긴 것이 없다 — 넣는 것은 규칙이 아니라 **새로 고르는 일**이다. 그리고 `条` 가 F03 둘째 묶음("부를 낱말이 아직 없어")에 서는 것에 그림 다섯(F03·F03a·F03c·Scene1a·Scene5a)이 기대고 있다(`design/SCREENS.md` 「이 그림들이 기대고 선 것」). **정하는 사람: 기획+디자인.** 넣으면 그 다섯을 다시 그려야 하고, 안 넣으면 예시 기사 셋째 문장이 왜 있는지가 같이 정리된다 |
