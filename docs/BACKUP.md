...
**표가 비어 있는 이유 (2026-09-20).** 아직 안 한 것이 아니라 못 하는 것이다. 둘이 걸려 있다.

- **복구할 파일을 아직 못 골랐다.** 예약 백업은 매일 UTC 18:17 에 도는데 프로젝트를 만든 뒤 아직 그 시각이 오지 않아, 크론이 만든 파일은 하나도 없다. 절차 C 의 1단계가 파일 내려받기다. 위 체크리스트의 수동 호출이 남긴 파일이 절차 C 에 쓸 만한지는 확인하지 않았다.
- **세션은 절차 C 를 끝까지 못 한다.** 2·3단계가 `DATABASE_URL_ADMIN` 을 요구하는데 세션에서 연결 문자열을 얻는 길이 없다(`docs/TEAM.md` 7장). 리허설은 연결 문자열을 쥔 사람이 돌린다.

먼저 확인할 것은 리허설이 아니라 **크론이 실제로 파일을 만드는지**다. 안 만들어지면 3층이 비어 있다는 뜻이고, 그건 리허설보다 큰 문제다.
