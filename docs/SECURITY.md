...
| C6 | 확인함 | `/api/recordings` 가 `access: "private"` 로 올리고, 키가 `voice/<user_id>/…` 라 계정별로 갈린다. DB 에는 URL 이 아니라 pathname 만 남는다 |
| C7 | 해당 없음 | 원본을 다시 들려주는 경로가 아직 없다. 생기는 순간 확인한다 |
| C8 | 확인함 | 업로드가 그 사용자의 `voice_retention_days` 를 읽어 `audio_expires_at` 을 정한다. 30 은 행이 없을 때의 대비값일 뿐이다 |
| C9 | **못 지킴** | 만료된 오브젝트를 지우는 주기 작업이 **없다**. `vercel.json` 의 크론은 백업 하나고 `src/app/api/cron/` 에도 백업뿐이다 (3장 L4) |
| C10 | 확인함 | `pitch` 는 `recordings` 행에 있고 오브젝트와 따로 산다 |
| C11 | 해당 없음 | 설정에 보관 기간을 바꾸는 행이 아직 없다 |
| C12 | **못 지킴** | 계정 삭제가 스토리지를 건드리지 않는다 (3장 L4) |
| C13 | **못 지킴** | `/api/recordings` 가 `card_id` 의 주인을 확인하지 않는다. RLS 도 `user_id` 만 본다 (3장 N3) |
...
등급 셋에 정확히 들어맞지 않는 것은 더 무거운 쪽에 둔다. "지운다고 한 것이 안 지워진다"는 셋 중 어디에도 꼭 맞지 않는다.

### S2 · 샘 — 로그인 화면이 다른 사이트로 보내진다

`next` 파라미터를 거르는 가드가 네 곳에 같은 모양으로 있다 — `src/app/page.tsx`, `src/app/onboarding/kana/page.tsx`, `kana/module/page.tsx`, `kana/actions.ts`.

```ts
next && next.startsWith("/") && !next.startsWith("//") ? next : "/today"
```

`//` 는 막지만 `/\` 는 통과한다. 브라우저와 WHATWG URL 파서는 특수 스킴에서 백슬래시를 `/` 로 읽으므로 `/\evil.com` 은 `//evil.com` 이 되고 결국 `http://evil.com/` 이다. 확인했다:

```
new URL("/\evil.com", "http://localhost:3000/")  →  http://evil.com/
```

거른 값은 네 곳 모두 `redirect()` 로 그대로 들어간다. `page.tsx` 에서는 `<GoogleSignIn callbackURL={to} />` 로도 간다. 로그인한 사용자가 `/?next=/\evil.com` 을 열면 그대로 밖으로 나간다.

데이터가 새는 것은 아니다. 진짜 도메인을 미끼로 쓰는 문이고 사용자가 한 명이어도 걸리므로 보수적으로 샘에 둔다.

고치는 것은 한 곳이면 된다 — 네 곳이 같은 코드를 복사해 두었으니 헬퍼 하나로 합치고 백슬래시를 막는다.

```ts
export function safeNext(next: string | undefined): string {
  return next && /^\/[^/\\]/.test(next) ? next : "/today";
}
```

### L4 · 잃음 — 음성 원본이 지워지지 않는다

`/api/recordings` 가 원본 오디오를 Blob 에 올리고 `audio_expires_at` 을 정한다. 그런데 **그 시각이 지나도 지우는 것이 없다.** 크론은 백업 하나뿐이다. 라우트 주석은 "만료 삭제는 cron 이 한다"고 적지만 그 cron 이 없다.

계정 삭제는 더 나쁘다. `POST /api/account/delete` 는 스토리지를 건드리지 않고 `users` 행만 지운다. CASCADE 가 `recordings` 행을 지우면서 `audio_object_key` 도 함께 사라진다. **키가 사라지면 어떤 오브젝트를 지워야 하는지 알 길이 영영 없다.** 지웠다고 응답한 뒤 주인 없는 녹음이 스토리지에 남는다.

`CLAUDE.md` 의 "원본은 일정 기간 뒤 삭제", "계정 삭제 시 백업 사본까지 삭제" 가 둘 다 지금 성립하지 않는다.

고치는 순서가 중요하다(C9·C12).

- 계정 삭제는 **스토리지를 먼저** 지우고 그다음 DB 를 지운다. 키가 `voice/<user_id>/…` 라 접두사 하나로 끝난다. 스토리지 삭제가 실패하면 성공으로 응답하지 않는다.
- 만료 크론은 오브젝트를 지운 **뒤에** `audio_object_key` 를 NULL 로 만든다. 반대로 하면 실패한 삭제가 고아로 남고 남았다는 사실조차 DB 에 없다.
...
`/api/recordings` 도 같은 모양이다. `card_id` 를 폼에서 받아 그대로 넣고, 주인인지 보지 않는다. 남의 카드에 내 녹음을 매달 수 있다. 남의 것이 읽히지는 않는다.

제안: INSERT 정책에 노드 조건을 더한다. `recordings` 는 `card_id`·`chunk_id` 에 같은 조건을 건다.
