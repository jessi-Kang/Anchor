...
| DB 작업 경로 | 이 환경의 컨테이너는 **HTTPS 만 나간다. 5432 가 막혀 있다** — `psql` 은 설치돼 있어도 Neon 에 닿지 못하고 `pnpm db:migrate` 도 같은 벽에 막힌다. 그래서 DB 에 닿는 길은 **Neon MCP 하나뿐**이다. 조회도 마이그레이션 적용도 그 길로 한다 |
