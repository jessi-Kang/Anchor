...
/*
  **「앵커가 그 글자의 소리를 품는가」는 여기서 안 잰다** — `anchor-shape.test.ts` 로 옮겼고,
  거기서는 **두음법칙을 안 봐준다**(기획 `dfa1ea1`). 料/요리(옛 값)는 소리를 "요" 로 부르는데 카드는
...
test("두음법칙은 낱말 첫머리에서만 편다", () => {
  ...
  // 낱말이 「양」으로 부르는 글자(사전은 「량」) — 접지 않으면 멀쩡한 값이 틀린 것으로 잡힌다.
  // 표의 643개는 이제 접지 않아도 다 닿지만(両 은 「차량」으로 갔다) 규칙 자체는 여기서 잰다.
