 ...
 *
 * **이름 길이의 바닥은 두 글자다.** 처음엔 넷으로 잡았는데, 그 문턱이 **이 레포에서 제일 무거운
 * 인용 하나를 통째로 안 보고 있었다** — `CLAUDE.md` 머리의 「근간」(어느 문서가 이기는지를 정하는
 * 줄)이다. 둘로 내리면 그 하나가 들어오고 **새로 딸려 오는 헛것은 없다**(PM 축이 재서 넘겼고
 * 다시 셌다: 길이 1 까지 내려도 늘어나는 것은 그 하나뿐).
 *
 * **한 글자는 그대로 안 센다.** 이 레포의 한 글자 「」는 **16종 33회**(2026-09-22)인데 대부분
 * **한자와 소리**다(「김」5·「류」4·「역」4·「량」3…). 세기 시작하면 그날부터 헛것이 쏟아진다.
 *
 * **다만 「다 한자·소리다」는 참이 아니다.** PM 축이 열어서 반례를 냈다 — `design/SCREENS.md`
 * 의 「뒤」는 **제목을 가리키고**, 「홈」은 화면 라벨, 「틀」은 낱말이다. **바닥을 둘로 두는
 * 근거는 「한 글자가 이름일 리 없다」가 아니라 「오늘 파일 이름 곁에 선 한 글자가 없다」다** —
 * 앞엣것은 없는 안전을 있다고 말하는 줄이고, 그건 썩은 줄 번호보다 나쁘다.
 *
 * **안 재는 줄에 이 바닥도 같이 적는다** — 정규식 안에만 있으면 출력만 읽는 사람에게는
 * 「안 재는 것 목록에 없는 안 재는 것」이 된다. 적되 **언제 무너지는지까지** 적는다.
 *
...
console.log("안 재는 것 ②: 한 글자 이름. 오늘 파일 이름 곁에 선 한 글자 이름은 없다 — 서는 날 이 줄이 틀린다.");
