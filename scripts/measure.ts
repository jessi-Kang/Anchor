  ...
  */
  ...
  if (fb || unknown) {
    ...
    if (fb) {
      ...
      /*
        전에 여기엔 "이 수는 덮어쓰이지 않아 안 줄어든다" 고 적혀 있었다. 문이 둘 다 닫혀 있었기
        때문이다 — `hasEnglish` 가 폴백 행을 "영어가 있다" 로 읽어 F17 로 안 보냈고, 거기까지 가도
        `saveEnglish` 가 `btrim(text) = ''` 일 때만 썼다. 그래서 **고칠 방법이 없는 수**였고,
        "다시 열어라" 고 적는 것은 없는 방법을 알려 주는 것이라 안 적었다.

        **둘 다 열렸다.** `hasEnglish` 가 `content_source === "fallback"` 이면 false 를 돌려주고
        (`lib/db/chunks.ts:67`), `saveEnglish` 의 WHERE 가 `meta ->> 'content_source' = 'fallback'`
        을 같이 받는다(`:233`). 이제 그 덩어리를 열면 낫는다. 그래서 문장을 바꾼다 —
        **시간이 아니라 행동으로 줄어드는 수**다.
      */
      console.log("  이 수는 시간이 지난다고 안 줄어든다. 그 덩어리를 다시 열어야 줄어든다.");
    }
    if (unknown)
  ...
  if (denom === 0) {
    ...
    /*
      ...
      **70.0% 로 통과 기준(자료 재만남 인식률 70%)과 같은 수**라, 잘린 줄은 「통과했다」로 읽힌다.
