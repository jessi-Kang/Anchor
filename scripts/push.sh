...
if ! pnpm verify >"$log" 2>&1; then
  ...
  #
  # **여기 살리는 명령을 박아 두지 않는다.** 한동안 `pg_ctlcluster 16 main start` 를 찍었는데,
  # 2026-09-22 열한 번째 회수 때 그대로 따라 쳤더니 `Removed stale pid file.` 만 찍고 끝났고
  # 앱이 쓰는 DB(5433)는 그때도 죽어 있었다. **이 레포에 DB 를 세우는 길이 둘**이라 그 줄은
  # 대마다 **다른 것을 살리고 성공이라고 말한다.** 안 맞는 안내는 다음 줄을 찾게 만들지만
  # **성공처럼 보이는 안내는 거기서 멈추게 한다** — 그래서 더 나쁘다.
  #
  # 그렇다고 명령 둘을 찍고 고르라고 하지도 않는다. 고르는 값이 하나 더 생기면 그 값이 또 틀릴
  # 자리다(위의 「검사를 붙이면」과 같은 까닭). **가르는 줄이 있는 곳 하나**를 가리킨다.
  # 연결 문자열은 여기서 안 찍는다 — 포트를 보려면 그 값을 봐야 하는데, 로그에 남길 값이 아니다.
  printf '몇 분이면 Postgres 가 아니라 컨테이너가 회수된 것이다 — 살리고 이 명령을 다시 돌려라.\n'
  printf '살리는 줄은 대마다 다르다: db/README.md 의 「살리기 전에 내 DB 가 어느 포트인지부터 본다」.\n'
  exit 1
fi
