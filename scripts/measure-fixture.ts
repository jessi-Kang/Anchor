...
async function main() {
  ...
  try {
    ...
    // 본문이 서로 달라야 "같은 글을 다시 넣었는가" 가 뜻을 갖는다. 재투입만 본문이 같다.
    const SRC_BODY = "카드를 푼 그 글. 協収談報誠拠支承継績暫謹頒妥.";
    const src = await input("ja", "카드를 푼 자료", SRC_BODY, 12);
    const again = await input("ja", "자격을 갖춘 새 자료", "진짜 새 글. 協収談報誠拠支承継績妥.", 4);
    const later = await input("ja", "그 뒤의 자료", "또 다른 새 글. 承.", 2);
    /*
      **실패 케이스 7 — 카드를 푼 그 글을 며칠 뒤 다시 넣었다.** 본문이 `src` 와 **글자 그대로 같다.**
      자격(착지 +3일)은 갖췄지만 재만남으로 세면 안 된다 — 글자를 알아본 것인지 그 글을 기억한
      것인지 가를 수 없다 (MEASURE 1장).

      **이게 거르는 순서를 시험한다.** 아래에서 支 는 D-6(재투입, 안 열었다)과 D-4(진짜 새 자료,
      열었다) 두 곳에서 만난다. 먼저 세고 나중에 거르는 구현은 D-6 을 첫 재만남으로 잡은 뒤
      무효라고 버려서 **支 의 표본을 통째로 잃는다.**

      그때 나오는 숫자를 실제로 재 봤다 — **분모 9 · 분자 7 = 77.8%.** 支 가 "열었다" 쪽이라
      분자는 그대로인데 분모만 줄어서 **비율이 위로 부푼다.** 표본 하나를 잃는 것보다 나쁘다:
      숫자가 사라지는 게 아니라 좋아 보이는 쪽으로 틀린다. 무효를 먼저 버리면 D-4 가 첫 재만남이라
      분모 10 · 분자 7 = 70.0% 그대로다.
    */
    const reinput = await input("ja", "같은 글을 다시 넣은 자료", SRC_BODY, 6);
    ...
    for (const ch of main10) {
      ...
      // 支 는 재투입 자료에서 **먼저**(D-6) 만난다. 그 줄이 진짜(D-4)를 밀어내면 안 된다.
      if (ch === "支") await encounter(id, reinput, true, 6);
    }
    ...
    /*
      **실패 케이스 8 — 폴백 문안.** 영어를 못 만들면 사용자가 쓴 한국어가 그대로 덩어리에 들어가고,
      화면이 그걸 영어 목소리로 읽어 그 소리가 1회차 기준선으로 굳는다. 거리는 계산되지만
